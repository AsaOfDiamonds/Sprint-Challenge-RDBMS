...
1. Explain the difference between `RDBMS` and `SQL`.
  A Relational Database Management System (RDBMS) is the database software. SQL is the laguage used
  to interact with that RDBMS system.

2. Why do tables need a `primary key`?
  A primary key is the unique identifier for a record in a table. There may be more than one Trevor Fehrman 
  so the primary key can be used to make each Trevor Fehrman distinctly identified. As a result each table needs
  a primary key to distinctly identify that table.

3. What is the name given to a table column that references the primary key on another table.
  foreign key

4. What do we need in order to have a _many to many_ relationship between two tables.
We need to create a third table that uses foreign keys that point to the primary key on the other tables.
Many to many is really smoke and mirrors.
